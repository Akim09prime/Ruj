...
export const AIExpert: React.FC = () => {
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResponse("Eroare la conectarea cu expertul AI.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-surface border border-border p-12 shadow-sm max-w-4xl mx-auto">
      ...
      <form onSubmit={handleAsk} className="flex flex-col space-y-4 mb-10">
        ...
        >
          {loading ? 'Consultare...' : 'Cere Expertiză'}
        </button>
      </form>

      {response && (
        <div className="animate-fade-in">
          <div className="prose dark:prose-invert max-w-none text-muted leading-relaxed bg-surface-2 p-8 border-l-2 border-accent">
            {response}
          </div>
          {sources.length > 0 && (
            <div className="mt-8 pt-6 border-t border-border">
              <h4 className="text-[10px] uppercase tracking-widest font-bold text-accent mb-4">Surse de Documentare:</h4>
